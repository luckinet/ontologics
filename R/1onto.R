...
setValidity("onto", function(object) {
  ...
  if (!.hasSlot(object = object, name = "classes")) {
    ...
    if (!all(c("level", "label", "description", "has_broader", "has_close_match", "has_narrower_match", "has_broader_match", "has_exact_match") %in% names(object@classes$harmonised))) {
      errors <- c(errors, "the ontology must have a table of harmonised classes with the columns 'level' and 'label', 'description', 'has_broader' and 'has_close_match', 'has_narrower_match'', 'has_broader_match' and 'has_exact_match'.")
    }
