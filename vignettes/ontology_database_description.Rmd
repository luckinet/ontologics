...
__uri_prefix__ (*optional*): Prefix of the resources of the ontology. See also [https://www.w3.org/TR/turtle/] at 2.4 IRIs. If no `uri_prefix` for the harmonised resources of the ontology is given, the `export_as_rdf()` function, inserts the placeholder value "http://www.example.org/ontologics/". If non-harmonised sources are provided without a `uri_prefix` the are ignored by the `export_as_rdf()` function. 
