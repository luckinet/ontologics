...
The IRI-IDs (IRIs from here) do not have to resolve to an actual resource on the Web, but it is recommended . If not possible, at least the prefix should resemble an actual Web page, as it is the case for the example.
...
Harmonised concepts relate to other harmonized concepts via the SKOS semantic relation `skos:broader`. Harmonised classes relate to other harmonized classes via the SKOS semantic relation `skos:broader`. Harmonised Concepts relate to external concepts via the SKOS mapping properties. Harmonised Classes relate to external Classes via the SKOS mapping relations.