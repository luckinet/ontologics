...
This tells us that that both 'NUTS' and 'Wheat' don't seem to be missing from the ontology. Moreover, we  see that 'Wheat' is a *class* and not a *crop* and 'NUTS' doesn't have any *broader* concept. We should probably check the hierarchical structure of the ontology for these two concepts as well. For 'NUTS' we can ask directly for the respective *code* and for 'Wheat' we would ask for the *broader* code. We see already here that the code has the same length, so 'NUTS' and whatever the parent of 'Wheat' are should be at the same hierarchical level.

```{r get_concept tree}
get_concept(x = newConcepts[1,], tree = TRUE, ontology = crops)[,1:6] %>% 
  kable()

get_concept(x = newConcepts[2,], tree = TRUE, ontology = crops)[,1:6] %>% 
  kable()
```
