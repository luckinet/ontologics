...
get_concept(x = tibble(label = "Wheat"), tree = TRUE, ontology = crops) %>% 
  kable()
# ... ok, then let's specify that class and re-run new_concept
crops <- new_class(new = "cultivar", target = "crop", harmonised = TRUE,
...
```{r new ontology}
get_concept(x = tibble(label = "Wheat"), tree = TRUE, mappings = TRUE, ontology = crops)[,1:6] %>% 
  kable()

get_concept(x = tibble(label = "NUTS"), tree = TRUE, mappings = TRUE, ontology = crops)[,1:6] %>% 
  kable()

get_concept(x = tibble(label = "FRUIT"), tree = TRUE, mappings = TRUE, ontology = crops)[,1:6] %>% 
  kable()
```
