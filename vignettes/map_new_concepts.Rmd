...
newConcepts <- tibble(label_en = c("Wheat", "NUTS", "Avocado"))
```
...
missingConcepts <- get_concept(x = newConcepts, missing = TRUE, ontology = crops)
...
```{r get_concept}
get_concept(x = newConcepts[1:2,], ontology = crops) %>% 
  kable()
```
...
```{r get_concept tree}
get_concept(x = newConcepts[1,], tree = TRUE, ontology = crops) %>% 
  kable()

get_concept(x = newConcepts[2,], tree = TRUE, ontology = crops) %>% 
  kable()
```
...
```{r set_concept}
broaderConcepts <- get_concept(x = tibble(label_en = c("Wheat", "Tropical and subtropical Fruit")), 
                               ontology = crops)
...
```{r set_mapping}
toMap <- get_concept(x = tibble(label_en = c("wheat", "NUTS", "avocado")),
                     ontology = crops)

crops <- new_mapping(concept = toMap,
                     new = newConcepts %>% pull(label_en),
...
```{r}
broaderConcepts <- get_concept(x = tibble(label_en = c("wheat", "wheat")),
                               ontology = crops)
...
get_concept(x = tibble(label_en = "Wheat"), tree = TRUE, ontology = crops) %>% 
  kable()
...
get_concept(x = tibble(label_en = "NUTS"), tree = TRUE, ontology = crops) %>% 
  kable()

get_concept(x = tibble(label_en = "FRUIT"), tree = TRUE, ontology = crops) %>% 
  kable()
```
